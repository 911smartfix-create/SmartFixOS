...
import {
  ...
  ShoppingCart, Clock, AlertCircle, CheckCircle, XCircle, ArrowUpRight } from
"lucide-react";
...
const MetricCard = ({ title, value, subtitle, icon: Icon, trend, color = "cyan" }) => {
  ...
  return (
    <div className="bg-black/40 backdrop-blur-xl border border-cyan-500/20 rounded-2xl p-6 shadow-[0_8px_32px_rgba(0,168,232,0.2)] hover:border-cyan-500/50 transition-all group theme-light:bg-white theme-light:border-gray-200 theme-light:hover:border-cyan-500/50">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          ...
          <TabsContent value="sales" className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <MetricCard title="Ingresos Totales" value={`$${salesAnalysis.totalRevenue.toFixed(2)}`} subtitle={`${salesAnalysis.totalTransactions} transacciones`} icon={DollarSign} color="emerald" />
              <MetricCard title="Ticket Promedio" value={`$${salesAnalysis.avgTicket.toFixed(2)}`} subtitle="Por transacción" icon={TrendingUp} color="cyan" />
              <MetricCard title="IVU Recaudado" value={`$${(salesAnalysis.totalRevenue * 0.115).toFixed(2)}`} subtitle="11.5% de ventas" icon={FileText} color="purple" />
              <MetricCard title="Transacciones" value={salesAnalysis.totalTransactions} subtitle="Ventas completadas" icon={ShoppingCart} color="amber" />
            </div>
