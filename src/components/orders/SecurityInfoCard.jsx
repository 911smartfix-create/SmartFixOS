      ...
      <CardContent className="space-y-4">
        ...
        {hasPattern && (
          <div className="bg-black/30 border border-amber-500/20 rounded-lg p-3">
            ...
            {/* Grid visual del patrón */}
            <div className="grid grid-cols-3 gap-2 justify-items-center mb-3">
              {[0, 1, 2, 3, 4, 5, 6, 7, 8].map((idx) => {
                const isInPath = patternPath.includes(idx);
                const pathIndex = patternPath.indexOf(idx);
                ...
                return (
                  <div
                    ...
                    className={`w-10 h-10 rounded-full border-2 flex items-center justify-center transition-all ${
                      isInPath
                        ? "bg-cyan-600/80 border-cyan-300 shadow-lg shadow-cyan-600/30"
                        : "bg-black/40 border-white/10"
                    }`}
                  >
                    {isInPath && (
                      <span className="text-white text-xs font-bold">{pathIndex + 1}</span>
                    )}
                  </div>
                );
              })}
