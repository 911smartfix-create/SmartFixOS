...
import {
  ...
  ClipboardList, Shield, MessageSquare, Link, Loader2, Download } from
"lucide-react";
          ...
          <CardTitle className="text-white flex items-center justify-between theme-light:text-gray-900">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                disabled={saving}
                className="bg-gradient-to-r from-emerald-600 to-lime-600 hover:from-emerald-700 hover:to-lime-700 shadow-[0_4px_16px_rgba(16,185,129,0.4)]">
                    ...
                    isAdded ? "border-emerald-500/50" : "border-white/10 hover:bg-black/60"} theme-light:bg-gray-50 theme-light:border-gray-200`
                      ...
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <div className="text-right flex items-center gap-3">
                          ...
                          className={`disabled:opacity-50 disabled:cursor-not-allowed h-8 px-3 ${
...
export default function WorkOrderPanel({ orderId, onClose, onUpdate, onDelete, panelVersion = "v9" }) {
  ...
  const sendStatusChangeEmail = async (newStatusId, previousStatusId) => {
    ...
    const skipEmailStates = ["reparacion_externa", "waiting_order", "pending_order", "intake", "diagnosing"];
        ...
        try {
          if (order.assigned_to && order.assigned_to !== me?.id) {
            const statusLabels = {
              ...
              picked_up: "Entregado",
              delivered: "Entregado", // Added delivered
            ...
            await NotificationService.createNotification({
              ...
              priority: nextId === "ready_for_pickup" || nextId === "delivered" ? "high" : "normal", // Adjusted priority
              metadata: {
          ...
          if (nextId === "ready_for_pickup" || nextId === "delivered") {
            ...
            for (const targetUser of eligibleUsers) {
              await NotificationService.createNotification({
                ...
                type: nextId === "delivered" ? "order_completed" : "order_ready",
                title: nextId === "delivered" ? `✅ Orden #${order.order_number} entregada` : `✅ Orden #${order.order_number} lista`,
                body: `${order.customer_name} - ${order.device_brand || ""} ${order.device_model || ""}`,
                relatedEntityType: "order",
                relatedEntityId: order.id,
                relatedEntityNumber: order.order_number,
    ...
    <>
      <div ref={panelRef} className="fixed inset-0 z-[60] bg-black/95 backdrop-blur-sm overflow-hidden kb-aware-panel" data-keyboard-aware>
        ...
        <div className="h-full flex flex-col bg-gradient-to-br from-[#0D0D0D] to-[#1A1A1A] theme-light:bg-gray-50">
          {/* ✅ HEADER RESPONSIVE */}
          <div className="flex-shrink-0 border-b border-gray-800 bg-black/60 backdrop-blur theme-light:border-gray-200 theme-light:bg-white/80">
            <div className="max-w-[1800px] mx-auto px-3 sm:px-6 py-3 flex items-center justify-between gap-3">
              <div className="flex items-center gap-2 sm:gap-4 min-w-0">
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={handleClose}
                  className="text-gray-400 hover:text-white flex-shrink-0 theme-light:text-gray-600 theme-light:hover:text-gray-900">
                ...
                <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                    <h1 className="text-base sm:text-xl font-bold text-white truncate wo-header-title theme-light:text-gray-900">
                      {order.order_number}
                    </h1>
                    <Badge className={`${getStatusConfig(order.status).colorClasses} text-xs whitespace-nowrap`}>
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  onClick={async () => {
                    const businessInfo = await getBusinessInfo();
                    downloadWorkOrderPDF(order, businessInfo);
                    toast.success("📄 PDF descargado");
                  }}
                  className="border-emerald-600/30 text-emerald-400 hover:bg-emerald-600/20 flex-shrink-0 theme-light:border-emerald-300 theme-light:text-emerald-700 theme-light:hover:bg-emerald-50">
                ...
                <Button
                  ...
                  disabled={loading}
                  className="border-cyan-600/30 text-cyan-400 hover:bg-cyan-600/20 flex-shrink-0 theme-light:border-cyan-300 theme-light:text-cyan-700 theme-light:hover:bg-cyan-50">
                  <RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} />
                  <span className="hidden sm:inline sm:ml-2">Actualizar</span>
                </Button>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto">
              <div className="max-w-[1800px] mx-auto px-3 sm:px-6 py-4 sm:py-6 space-y-4 sm:space-y-6 wo-panel-content">

                {/* ✅ ESTADO - GRID RESPONSIVE */}
                <Card className="bg-gradient-to-br from-cyan-600/10 to-emerald-600/10 border-cyan-500/20 theme-light:bg-white theme-light:border-gray-200">
                  <CardHeader className="border-b border-cyan-500/20 pb-3 sm:pb-4 theme-light:border-gray-200">
                    <CardTitle className="text-white flex items-center gap-2 text-base sm:text-lg theme-light:text-gray-900">
                      <div className="w-2 h-2 rounded-full bg-cyan-500 animate-pulse" />
                      Estado de la Orden
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="pt-4 sm:pt-6">
                    <div className="mb-4 sm:mb-6 p-3 sm:p-4 rounded-lg bg-gradient-to-r from-cyan-600/20 to-emerald-600/20 border-2 border-cyan-500/50 theme-light:from-cyan-50 theme-light:to-emerald-50 theme-light:border-cyan-300">
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          <p className="text-xs text-gray-400 mb-1 theme-light:text-gray-600">Estado Actual</p>
                          <p className="text-xl sm:text-2xl font-bold text-white theme-light:text-gray-900">
                            {getStatusConfig(status).label}
                          </p>
                        </div>
                        <div className={`px-3 sm:px-4 py-1 sm:py-2 rounded-full border ${getStatusConfig(status).colorClasses}`}>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-3 wo-status-grid">
                    ...
                    <div className="mt-4 sm:mt-6 pt-4 sm:pt-6 border-t border-white/10 theme-light:border-gray-200">
              ...
              <Card className="bg-[#0F0F12] border-white/10 p-5 theme-light:bg-white theme-light:border-gray-200">
                ...
                <Card className="p-3 sm:p-4 bg-gradient-to-br from-cyan-600/10 to-emerald-600/10 border-cyan-500/20 theme-light:bg-white theme-light:border-gray-200">
                  <h3 className="text-white font-semibold mb-3 text-sm sm:text-base flex items-center gap-2 theme-light:text-gray-900">
                    <ClipboardList className="w-5 h-5 text-cyan-500 theme-light:text-cyan-600" />
                    Resumen
                  </h3>
                ...
                <Card className="p-3 sm:p-4 bg-gradient-to-br from-lime-600/10 to-emerald-600/10 border-lime-500/20 theme-light:bg-white theme-light:border-gray-200">
                  <h3 className="text-white font-semibold mb-3 text-sm sm:text-base flex items-center gap-2 theme-light:text-gray-900">
                    <Shield className="w-5 h-5 text-lime-500 theme-light:text-lime-600" />
                    Seguridad
                  </h3>
                  {!hasPassword && !hasPin && !hasPattern ?
                <div className="text-[13px] text-gray-400 theme-light:text-gray-600">—</div> :
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  ...
                  <SecurityItem
                    ...
                    visibleValue={safeAtob(sec.device_pin)} />
                      ...
                      {(hasPattern || sec.pattern_vector) &&
                  <div className="sm:col-span-2">
                          <div className="text-[11px] text-gray-400 theme-light:text-gray-600">Patrón (Android)</div>
                          <div className="mt-2">
                            {sec.pattern_image ? (
                              <img
                                src={sec.pattern_image}
                                alt="Patrón"
                                className="w-32 h-32 sm:w-44 sm:h-44 object-contain rounded-md border border-white/10 bg-black/40 theme-light:border-gray-200 theme-light:bg-gray-50" />
                            ) : sec.pattern_vector ? (
                              <div className="bg-black/40 border border-white/10 rounded-lg p-4 theme-light:bg-gray-50 theme-light:border-gray-200">
                                <p className="text-white font-mono text-sm theme-light:text-gray-900">
                                  {sec.pattern_vector}
                                </p>
                                {sec.pattern_length && (
                                  <p className="text-xs text-gray-400 mt-2 theme-light:text-gray-600">
                                    {sec.pattern_length} puntos conectados
                                  </p>
                                )}
                              </div>
                            ) : null}
                ...
                <Card className="bg-[#0F0F12] border-white/10 theme-light:bg-white theme-light:border-gray-200">
                  <CardHeader className="border-b border-white/10 pb-3 sm:pb-4 theme-light:border-gray-200">
                    <CardTitle className="text-white flex items-center gap-2 text-base sm:text-lg theme-light:text-gray-900">
                ...
                <Card className="p-3 sm:p-4 bg-gradient-to-br from-cyan-600/10 to-emerald-600/10 border-cyan-500/20 theme-light:bg-white theme-light:border-gray-200">
                  <h4 className="text-white font-semibold mb-3 flex items-center gap-2 text-sm sm:text-base theme-light:text-gray-900">
