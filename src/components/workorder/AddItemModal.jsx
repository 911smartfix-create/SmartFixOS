        ...
        <div className="bg-gradient-to-br from-[#111114] to-[#0D0D0D] border border-cyan-500/20 rounded-2xl w-full max-w-4xl max-h-[85vh] flex flex-col shadow-[0_24px_80px_rgba(0,168,232,0.5)] theme-light:bg-white theme-light:border-gray-200">
          ...
          <div className="flex gap-2 px-6 pt-4">
            ...
            <Button
              ...
              onClick={() => setTab("offers")} className="bg-background text-slate-900 px-4 py-2 text-sm font-medium rounded-md inline-flex items-center justify-center gap-2 whitespace-nowrap transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0 border shadow-sm hover:bg-accent hover:text-accent-foreground h-9 border-orange-500/20 theme-light:border-orange-300">


              🏷️ Ofertas {products.filter((p) => p.discount_active && p.discount_percentage > 0 && (!p.discount_end_date || new Date(p.discount_end_date) >= new Date())).length > 0 &&
