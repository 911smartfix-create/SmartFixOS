...
export default function BrandIconGrid({ formData, updateFormData, onAutoAdvance, config }) {
  ...
  const loadBrands = async () => {
    ...
    try {
      if (useNewSelector) {
        ...
        const catalogBrands = DEVICE_CATALOG.brands.map(b => ({
          id: b.id,
          name: b.label,
          icon_url: b.icon,
          active: true
        ...
        let rows = await base44.entities.Brand.filter({ active: true }).catch(async () => {
            // fallback a list si filter no existiera
            const all = await base44.entities.Brand.list();
            return (all || []).filter((b) => b.active !== false);
        });
        rows = Array.isArray(rows) ? rows : [];
        
        // 👈 B) Orden: primero por order, luego alfabético
        rows.sort((a, b) => {
      ...
      setBrands([]); // Fallback to empty if catalog or API fails
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSelect = (brand) => {
    // PROMPT 4 & 7: Guardar la marca completa
    updateFormData("device_brand", brand);
    
    // Clear subsequent selections when a new brand is chosen
