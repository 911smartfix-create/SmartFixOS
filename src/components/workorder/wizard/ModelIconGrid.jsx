...
export default function ModelIconGrid({ formData, updateFormData, onAutoAdvance, config }) {
  ...
  const loadModels = async () => {
    try {
      setLoading(true);
      // Cargar modelos de BD por marca/familia; mantener campos existentes
      const data = await base44.entities.DeviceModel.filter({
        brand_name: selectedBrand.name || selectedBrand.label || selectedBrand,
        family_name: selectedFamily.name || selectedFamily.label || selectedFamily,
        active: true
      });
